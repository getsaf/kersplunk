...
`(logDetails: object) => object`

Takes in the original details and returns the "enhanced" details.

```typescript
logger.interceptor = details => ({
  ...details,
  icon: details.logType === 'error' ? '💩' : '😎',
});
